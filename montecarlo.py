...
ms.expand_region(original_chunk, 5)
...
def export_results(doc, run_idx):
    chunk = doc.chunk
...
def simulation_run(run_idx):
    ...
    # chunk = duplicate_chunk(zero_error_chunk, f"simu_chunk_{run_idx:04d}")
    ...
    # export_thread.start()
    # export_thread.join()
    export_results(run_doc, run_idx)
...
if run_parallel:
    ...
    pool.map(simulation_run, range(num_randomisations))
else:
    for run_idx in range(0, num_randomisations):
        simulation_run(run_idx)
